...
public class sintactico {
    ...
    private String tipoAString(int tipoToken) {
        switch (tipoToken) {
            case MatrizTipos.T_INT:
                return "int";
            case MatrizTipos.T_FLOAT64:
                return "float64";
            case MatrizTipos.T_STRING:
                return "string";
            case MatrizTipos.T_BOOL:
                return "bool";
            default:
                return "desconocido";
        }
    ...
    private int getTipoDeToken(nodo token) {
        ...
        switch (token.token) {
            case 100: // Identificador
                ...
                return MatrizTipos.T_ERROR;
            case 101:
                return MatrizTipos.T_INT;
            case 102:
                return MatrizTipos.T_FLOAT64;
            case 103:
                return MatrizTipos.T_STRING;
            case 213:
            case 214: // true, false
    ...
    private int obtenerValorConstante(String lexema, int tipo) {
        if (tipo == MatrizTipos.T_INT) {
            try {
    ...
    private int expresion() {
        ...
        while (p != null && (p.token == 104 || p.token == 105)) { // + -
            ...
            if (tipoIzq == MatrizTipos.T_ERROR || tipoDer == MatrizTipos.T_ERROR) {
                return MatrizTipos.T_ERROR;
            }
    ...
    private int termino() {
        ...
        String operandoIzq = "";
        if (!polaca.isEmpty()) {
            operandoIzq = polaca.get(polaca.size() - 1);
        }

        while (p != null && (p.token == 106 || p.token == 107)) { // "*" "/"
            nodo operador = p;
            String operadorString = operador.lexema;
            int tokenOperador = operador.token;
            ...
            String operandoDer = "";
            if (!polaca.isEmpty()) {
                operandoDer = polaca.get(polaca.size() - 1);
            }
            ...
            int tipoResultante = MatrizTipos.verificar(operadorString, tipoIzq, tipoDer);
            if (tipoResultante == MatrizTipos.T_ERROR) {
                imprimirErrorSemantico("Operador '" + operadorString + "' no se puede aplicar a operandos de tipo '"
                        + tipoAString(tipoIzq) + "' y '" + tipoAString(tipoDer) + "'", operador.renglon);
                return MatrizTipos.T_ERROR;
            }
    ...
    private int expresion_logica_simple() {
        if (p != null && p.token == 116) { // '!'
            ...
            if (tipoExpr == MatrizTipos.T_ERROR) {
                return MatrizTipos.T_ERROR;
            }
